...
| R-017 | Dragging a driven activity is meaningful | Dragging an activity that has predecessors applies a Start No Earlier Than (SNET) constraint, marked on the Start cell, and offers to record the move as lag on the driving link instead. Dragging it earlier than its logic allows adjusts the lag, since a constraint cannot. The constraint is removed from the date picker ("Remove constraint") | P1 |
